...
# NOTE:
#   1. The effect allele is encoded as 2
...
if (length(args) < 2) {
    ...
    warning("More than 6 arguments are given, only pick up first 6!\n")
}
...
if (!is.null(phenotype_file)) {
	...
	if (!is.null(args[6])) {
		phenotype_genotype_chosen$phenotype <- log10(as.double(phenotype_genotype_chosen$phenotype))
	} else {
		phenotype_genotype_chosen$phenotype <- as.double(phenotype_genotype_chosen$phenotype)
	}

    genotype_types <- unique(genotypes)
    genotype_type_n <- length(genotype_types)
    if (genotype_type_n == 2) {
        print(typeof(phenotype_genotype_chosen$phenotype))
        tt_res <- t.test(phenotype ~ genotype, data = phenotype_genotype_chosen)
        print(tt_res)
    } else {
        ...
        if (aov_summary[[1]]["genotype", "Pr(>F)"] <= 0.05) {
    ...
    g <- g + geom_boxplot(aes(x = genotype, y = phenotype, color = genotype))
    g <- g + geom_point(aes(x = genotype, y = phenotype, color = genotype))
