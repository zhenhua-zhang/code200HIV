...
pprcs <- function(flnm, dscd_cols=NULL, dscd_rows=NULL, kept_cols=NULL,
          kept_rows=NULL, trps=TRUE, as_idx="id", dscd_idx=FALSE,
          zipped=FALSE, swhd=20) {
...
plt_mht <- function(qtls_lst, use_cols=NULL, opt_prfx="qtl_mapping",
  ...
  if(is.null(use_cols)) {
    ...
    stop("The length of use_cols should be 4 ...\n")
  }
...
plt_dsg <- function(qtl_dtfm, pntp, gntp, gntp_ptn="*.dosage", gntp_zipped=TRUE,
          ...
          gntp_idx_col="id", gntp_dscd_idx=TRUE, opt_dir="reports/") {
  ...
  if(file_test("-d", gntp)) {
    ...
    for (flnm in flnmlst) {
      ...
      tmp_dtfm <- pprcs(
	    gntp_file, dscd_cols=gntp_blck_lst, trps=FALSE, as_idx=gntp_idx_col,
	    dscd_idx=gntp_dscd_idx
	  )
...
qtlrpt <- function(optlst) {
  ...
  ipt_tmp_flnm <- file.path(tmp_dir, "trfm_tmp.csv")
  pntp_idx_col <- optlst$pntp_idx_col
  if(file.exists(ipt_tmp_flnm)) {  ## Here removing id is enough
