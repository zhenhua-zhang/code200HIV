...
prcs_cfg <- function(iptfl = NULL) {
  if (is.null(iptfl)) {
    stop("iptfl is required but found NULL")
  }
  cfglst <- list()
  ...
  for (line in cfgs) {
    if (!startsWith(line, "#") && !stri_isempty(line)) {
      plst <- strsplit(line, "=")
      if (length(plst) != 1) {
        stop("The length of plst should be 1")
      }
      key <- plst[[1]][1]
      val <- plst[[1]][2]
      cfglst[[key]] <- val
...
# Preprocess
pprcs <- function(flnm, dscd_cols = NULL, dscd_rows = NULL, kept_cols = NULL, kept_rows = NULL, trps = TRUE, as_idx = "id", dscd_idx = FALSE, zipped = FALSE, swhd = 20) {
...
trfm_nmlt <- function(dtfm, dscd_cols = NULL, trfm_cols = NULL, trfm_mthd = "log2", idcol = "id", stclnm = TRUE) {
  ...
  if (!is.null(trfm_cols) && !is.na(trfm_cols)) {
    ...
    if (is.null(trfm_mthd)) {
      stop("When trfm_cols is not NULL, trfm_mthd is required.")
    }
...
plt_mht <- function(qtls_lst, use_cols = NULL, opt_prfx = "qtl_mapping", svfmt = "pdf", wdth = 20, hght = 10, drwzsc = FALSE, drwqq = FALSE) {
  ...
  manhattan(gwas_rslt, main = "Manhattan Plot (p-value)", ylab = "p-value(-log10)",
...
ckmd <- function(dtfm, thrshld = 0.5) {
  if (is.data.frame(dtfm)) {
    ...
    stop("I can only handle vector or data.frame!!!")
  }
}
...
dmptsvcf <- function(tspc, vcf_fl, vcf_rfg, opt_dir = "reports", kept_info = NULL, dscd_info = NULL) {
  ...
  tspc_vcf <- readVcf(TabixFile(vcf_fl), vcf_rfg, vcfprm)
  vcfrpt_path <- file.path(opt_dir, "report.vcf.gz")
  writeVcf(tspc_vcf, vcfrpt_path, index = TRUE)
}


# MaKe Top Snp Per Chromosome
mktspc <- function(qtl_dtfm, pvt = 5e-8, qtl_col = NULL) {
  #'
...
lddsg <- function(gntp, gntp_ptn = NULL, gntp_blck_lst = NULL, gntp_idx_col = NULL, gntp_dscd_idx = NULL) {
  if (file_test("-d", gntp)) {
    if (is.null(gntp_ptn))
    ...
    for (flnm in flnmlst) {
      ...
      tmp_dtfm <- pprcs(gntp_file, dscd_cols = gntp_blck_lst, trps = FALSE, as_idx = gntp_idx_col, dscd_idx = gntp_dscd_idx)
      gntp_dtfm <- rbind(gntp_dtfm, tmp_dtfm)
    }
  } else if (file_test("-f", gntp)) {
    gntp_dtfm <- pprcs(gntp_file, dscd_cols = gntp_blck_lst, trps = FALSE, as_idx = gntp_idx_col, dscd_idx = gntp_dscd_idx)
  } else {
...
mkrpt <- function(qtl_dtfm, pntp, gntp, gntp_ptn = "*.dosage", gntp_zipped = TRUE,
  vcf_fl = "dbSNP.vcf.gz", vcf_rfg = "hg19", pvt = 5e-8, qtl_col = NULL,
  gntp_blck_lst = NULL, gntp_idx_col = "id", gntp_dscd_idx = TRUE,
  opt_dir = "reports") {
  #' @title mkrpt(1)
  #' @description Create a report in .csv format for current mapping results
  #' @param qtl_dtfm A dataframe including QTL mapping results
  #' @param phtp A file, including phenotype information
  #' @param gntp A directory or file, including genotype information

  tspc <- mktspc(qtl_dtfm = qtl_dtfm, pvt = pvt, qtl_col = qtl_col)

  if (dim(tspc)[1] == 0) {
    cat("[WARN]  tspc(datafram top snp per chrom) is empty. Exit ...\n")
    return(NULL)
  }

  # Dump VCF records for the top SNPs per chromosome to drive
  dmptsvcf(tspc, vcf_fl = vcf_fl, vcf_rfg = vcf_rfg, opt_dir = opt_dir)

  # Fetch phenotypes
  # A dataframe with column as subjects, rows as measurements
  if (is.data.frame(pntp))
    use_pntp <- pntp[which(base::rownames(pntp) %in% unique(tspc$gene)), ]
  else
    stop("pntp should be a dataframe")

  # Fetch genotype dosages
  # TODO: Repeated code, should rewrite into a function
  gntp_dtfm <- lddsg(
    gntp = gntp, gntp_ptn = gntp_ptn, gntp_blck_lst = gntp_blck_lst,
    gntp_idx_col = gntp_idx_col, gntp_dscd_idx = gntp_dscd_idx
  )

  gntp_rwnm <- base::rownames(gntp_dtfm)
  use_gntp <- gntp_dtfm[which(gntp_rwnm %in% unique(tspc$snps)), ]

  # "snp", "chrom", "pos", "ref", "alt", "genotypes:individuls", "measurement", "pvalue", "gene(win_500kb)", "info"
  # base::rownames(gnpn) <- c(msmnt, snpid, "code")
  # dsg_dtfm, to draw dosage ~ genotype boxplot by plg_dsg()
  dsg_dtfm <- apply(tspc, 1, enc_gntp, use_gntp, use_pntp)

  # TODO: return a list with different processed results
  return(dsg_dtfm)
}
...
hdmtopt <- function(arg) {
  if (is.null(arg)) {
    cat("[WARN]  keyword argument arg is NULL\n")
    return(NULL)
  }
...
stwkspc <- function(tmp_dir = NULL, opt_dir = NULL, mode = "0755") {
  ...
  if (dir.exists(opt_dir)) {
    ...
    dirs <- c(
      "GenotypeLevelPlot", "QtlInformation", "LocusZoomPlot", "ManhattanPlot",
...
cknmlt <- function(optlst) {
  ...
  # Preprocessing
  pntp_dtfm <- pprcs(
    pntp_file, dscd_cols = pntp_dscd_cols, dscd_rows = pntp_dscd_rows, trps = FALSE,
  as_idx = pntp_idx_col, dscd_idx = pntp_dscd_idx
  )
...
qtlmp <- function(optlst) {
  ...
  if (file_test("-d", gntp_ipt)) {
    if (is.null(gntp_ptn)) stop("--genotypes is a directory, gntp_ptn is required")

    flnmlst <- list.files(gntp_ipt, pattern = gntp_ptn)
    if (length(flnmlst) == 0) stop("--genotypes is a directory, but NO file by: ", gntp_ptn)
    ...
    for (flnm in flnmlst) {
      ...
      qtl_dtfm <- rbind(qtl_dtfm, mkme(pntp_dtfm, gntp_dtfm, cvrt_dtfm)$all$eqtls)
    }
...
qtlrpt <- function(optlst) {
  ...
  if (file.exists(ipt_qtl_flnm)) qtl_dtfm <- pprcs(ipt_qtl_flnm, trps = FALSE, as_idx = NULL)
  else stop("Failed to find temporary file ", ipt_qtl_flnm)
  ...
  ipt_tmp_flnm <- file.path(tmp_dir, "trfm_tmp.csv")
  pntp_idx_col <- optlst$pntp_idx_col
  if (file.exists(ipt_tmp_flnm)) pntp_dtfm <- pprcs(ipt_tmp_flnm, dscd_cols = pntp_idx_col, trps = TRUE)
  else stop("Failed to find temporary file ", ipt_tmp_flnm)
  ...
  plt_dsg(
    ...
    gntp_ptn = gntp_ptn, pvt = qtlrpt_dsgplt_pvt, gntp_blck_lst = gntp_dscd_cols,
    opt_dir = optlst$opt_dsg_dir, svfmt = qtlrpt_dsgplt_svfmt,
    gntp_idx_col = gntp_idx_col, gntp_dscd_idx = gntp_dscd_idx
  )
}
...
test_fetch_vcf <- function() {
  ...
  # rowRanges(tspc_vcf, fixed = TRUE)  # Including parsed results from REF, ALT, QUAL, FILTER field
}
